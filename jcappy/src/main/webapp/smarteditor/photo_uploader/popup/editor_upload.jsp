...
// 폴더생성
File moveDir = new File(request.getServletContext().getRealPath(savePath));																								//복사될 디렉토리
if (!moveDir.exists()) {																										//복사될 경로가 없을경우 만든다.
	moveDir.mkdirs();
}
System.out.println(request.getServletContext().getRealPath(savePath));
MultipartRequest multi = new MultipartRequest(request, request.getServletContext().getRealPath(savePath), EDITOR_MAXSIZE, "utf-8",  new DefaultFileRenamePolicy());
